...
export default function MyComponent() {
  useEffect(() => {
    // Accessing DOM elements after component is mounted on the client side
    ...
    function updateProgressBar() {
      ...
      const sliderItems = Array.from(list.querySelectorAll(".item")); // Get the current visible order of items
      const activeItem =
        parseInt(sliderItems[0].querySelector(".title").getAttribute("data-item")) + 1; // The first visible item is the active one
    ...
    return () => {
      nextBtn.removeEventListener("click", () => handleSliderNavigation("next"));
      prevBtn.removeEventListener("click", () => handleSliderNavigation("prev"));
    };
